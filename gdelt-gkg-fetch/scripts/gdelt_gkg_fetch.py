#!/usr/bin/env python3
"""Fetch GDELT 2.0 gkg export files with retries, throttling, and rich logs."""
...
ENV_BASE_URL = "GDELT_GKG_BASE_URL"
...
def build_logger(level: str, log_file: str) -> logging.Logger:
    logger = logging.getLogger("gdelt-gkg-fetch")
...
def as_gkg_entry(size: int | None, md5: str, url: str) -> ExportFileEntry | None:
...
def parse_lastupdate_gkg(text: str, logger: logging.Logger) -> list[ExportFileEntry]:
    ...
    for line in text.splitlines():
        ...
        if not parsed:
            continue
        size, md5, url = parsed
        entry = as_gkg_entry(size=size, md5=md5, url=url)
        if entry is not None:
            results.append(entry)
    return sorted(results, key=lambda x: x.timestamp)


def iter_masterfile_gkg(
...
def resolve_latest_entries(
    *,
    ...
    entries = parse_lastupdate_gkg(text=text, logger=logger)
    if not entries:
        raise RuntimeError("No gkg export entry found in lastupdate.txt.")
...
def command_resolve_latest(args: argparse.Namespace) -> int:
    ...
    payload = {
        "ok": True,
        "source": "lastupdate",
        "latest_gkg_export": serialize_entry(latest),
...
def select_entries(args: argparse.Namespace, config: RuntimeConfig, logger: logging.Logger) -> list[ExportFileEntry]:
    ...
    candidates = sorted(
        iter_masterfile_gkg(text=text, start=start, end=end, logger=logger),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch GDELT 2.0 gkg export files via lastupdate/masterfilelist."
    )
    ...
    latest = sub.add_parser("resolve-latest", help="Resolve the latest gkg export file.")
    ...
    fetch = sub.add_parser("fetch", help="Download gkg export file(s).")
    ...
    fetch.add_argument(
        "--expected-columns",
        type=int,
        default=DEFAULT_GKG_EXPECTED_COLUMNS,
        help=f"Expected tab-separated column count for gkg rows. Default: {DEFAULT_GKG_EXPECTED_COLUMNS}.",
    )
    fetch.add_argument(
