...
## 迁移后扩展验收：GDELT Web NGrams

新增 `gdelt-web-ngrams-search` 不改变既有 21 项 EcoCouncil 来源清单；当前候选
`data-skill-migration-provenance.json` 更新为 21 项迁移结果加 1 项扩展，共 22 项。
它与旧候选共用 requirement/thin-file 校验和 copy/symlink 安装 smoke，实际 CLI
必须导出 `gdelt.web-ngrams/search` contract major 1。新功能发布前仅为本地候选。
CLI 负责 literal matching、GZIP/UTF-8 和配对 ID 校验、坏行 partial、缺失 blocked
及遗漏数量；Skill 验收检查是否明确单分钟覆盖、DOC 非等价性与三表语义分层。
真实检索要核对原始 quadgram 命中与 TOC URL，不能只以 HTTP 200 作为成功依据。

## 2026-09-07 Provider 可用性处置

- AirNow 的 CSV、路径规划和参数语义无误；故障是公开 CloudFront 边缘连接超时。CLI
  改用 AWS 明确返回的 `files.airnowtech.org`、`us-west-1` 区域化 S3 path-style endpoint，
  同一官方小时对象真实复测为 success/complete：1,014,422 bytes、4,431 输入行、5,818
  条 PM2.5/Ozone 记录，约 2.6 秒，未截断。
- GDELT DOC 在声明 pacing/retry 预算下无法让代表性文章与时间线模式稳定同时通过，
  因而在 CLI catalog 中 suspended；`gdelt-web-ngrams-search` 只在 literal 单分钟语义适配时
  独立选择，不作为静默降级。
- USBR legacy RISE、官方 EDR beta 和 `www.usbr.gov` 项目页在受支持执行环境均返回
  HTTP 200 `Request Rejected` 网关页。可访问的 WWDH EDR 是第三方缓存代理，无法无损
  实现当前显式 catalog-item 契约，因此不得静默替换。`usbr.rise` 与
  `usbr.project-records` 保留 discovery、fixture、Skill requirement 和恢复条件，但暂停
  data run，并由动态投影自动排除在 Auto Research 之外。
- 当前候选 CLI 为 0.0.61；22 项仓库级 migration provenance 已从同一打包候选重生并
  独立 verify。稳定 per-Skill requirement 未改变，因为 capability/operation contract
  major 与 required feature 没有变化。
