#!/usr/bin/env python3
"""Fetch Open-Meteo historical weather and soil data with validation and logs."""
...
ENV_BASE_URL = "OPEN_METEO_BASE_URL"
ENV_API_KEY = "OPEN_METEO_API_KEY"
ENV_TIMEOUT_SECONDS = "OPEN_METEO_TIMEOUT_SECONDS"
ENV_MAX_RETRIES = "OPEN_METEO_MAX_RETRIES"
ENV_RETRY_BACKOFF_SECONDS = "OPEN_METEO_RETRY_BACKOFF_SECONDS"
ENV_RETRY_BACKOFF_MULTIPLIER = "OPEN_METEO_RETRY_BACKOFF_MULTIPLIER"
ENV_MIN_REQUEST_INTERVAL_SECONDS = "OPEN_METEO_MIN_REQUEST_INTERVAL_SECONDS"
ENV_MAX_LOCATIONS_PER_RUN = "OPEN_METEO_MAX_LOCATIONS_PER_RUN"
ENV_MAX_DAYS_PER_RUN = "OPEN_METEO_MAX_DAYS_PER_RUN"
ENV_MAX_HOURLY_VARIABLES_PER_RUN = "OPEN_METEO_MAX_HOURLY_VARIABLES_PER_RUN"
ENV_MAX_DAILY_VARIABLES_PER_RUN = "OPEN_METEO_MAX_DAILY_VARIABLES_PER_RUN"
ENV_MAX_RETRY_AFTER_SECONDS = "OPEN_METEO_MAX_RETRY_AFTER_SECONDS"
ENV_DEFAULT_TIMEZONE = "OPEN_METEO_DEFAULT_TIMEZONE"
ENV_USER_AGENT = "OPEN_METEO_USER_AGENT"
...
DEFAULT_MAX_HOURLY_VARIABLES_PER_RUN = 12
DEFAULT_MAX_DAILY_VARIABLES_PER_RUN = 12
DEFAULT_MAX_RETRY_AFTER_SECONDS = 120
DEFAULT_TIMEZONE = "GMT"
DEFAULT_USER_AGENT = "open-meteo-historical-fetch/1.0"
...
def build_runtime_config(args: argparse.Namespace) -> RuntimeConfig:
    ...
    api_key = (args.api_key if args.api_key is not None else os.environ.get(ENV_API_KEY, "")).strip()
    ...
    max_hourly_variables_per_run = parse_non_negative_int(
        ...
        str(
            ...
            if args.max_hourly_variables_per_run is not None
                ...
                ENV_MAX_HOURLY_VARIABLES_PER_RUN, str(DEFAULT_MAX_HOURLY_VARIABLES_PER_RUN)
            )
        ),
    )
...
def build_logger(level: str, log_file: str) -> logging.Logger:
    logger = logging.getLogger("open-meteo-historical-fetch")
...
def build_request_query(
    *,
    ...
    if wind_speed_unit:
        query["wind_speed_unit"] = wind_speed_unit
    if precipitation_unit:
        query["precipitation_unit"] = precipitation_unit
    if config.api_key:
    ...
    *,
    ...
    if not isinstance(raw_values, list):
        add_record_issue(
            ...
            field="time",
            message=f"{section_name}.time must be a list.",
        )
        return [], None, None
    ...
    if exact_expected_count is not None and parsed_count != exact_expected_count:
        add_record_issue(
            ...
            field="time",
            message=(
                f"{section_name}.time length mismatch: expected {exact_expected_count}, "
                f"got {parsed_count}."
            ),
        )

    previous_key: datetime | date | None = None
    for offset, value in enumerate(raw_values):
        if not isinstance(value, str):
            add_record_issue(
                ...
                section=section_name,
                field="time",
                message=f"{section_name}.time[{offset}] must be a string.",
            )
            continue

        if section_name == "hourly":
            parsed = parse_hourly_timestamp(value)
            if parsed is None:
                add_record_issue(
                    collector,
                    ...
                    section=section_name,
                    field="time",
                    message=f"Cannot parse {section_name}.time[{offset}]={value!r}.",
                )
                continue
            compare_key: datetime | date = parsed
            compare_date = parsed.date()
        else:
            parsed_date = parse_daily_timestamp(value)
            if parsed_date is None:
                ...
                continue
            compare_key = parsed_date
            compare_date = parsed_date
        ...
        if compare_date < start_date_value or compare_date > end_date_value:
            add_record_issue(
                ...
                field="time",
                message=(
                    f"{section_name}.time[{offset}]={value!r} is outside requested date range "
                    f"{start_date_value.isoformat()}..{end_date_value.isoformat()}."
                ),
            )

        if previous_key is not None and compare_key <= previous_key:
            add_record_issue(
                collector,
                ...
                section=section_name,
                field="time",
                message=f"{section_name}.time is not strictly ascending at index {offset}.",
            )
        previous_key = compare_key
    ...
    *,
    ...
    if not isinstance(section, dict):
        add_record_issue(
            ...
            record_index=record_index,
            section=section_name,
            message=f"Missing or invalid {section_name} section.",
        )
        return summary

    units_key = f"{section_name}_units"
    units = record.get(units_key)
    if not isinstance(units, dict):
        add_record_issue(
            ...
            section=section_name,
            field=units_key,
            message=f"Missing or invalid {units_key} section.",
        )
        units = {}
    ...
    for variable in variables:
        if variable not in section:
            add_record_issue(
                ...
                section=section_name,
                field=variable,
                message=f"Requested variable {variable!r} is missing from {section_name}.",
            )
        ...
        if not isinstance(values, list):
            add_record_issue(
                ...
                section=section_name,
                field=variable,
                message=f"{section_name}.{variable} must be a list.",
            )
            variable_lengths[variable] = None
            continue
        ...
        if len(values) != len(time_values):
            add_record_issue(
                ...
                field=variable,
                message=(
                    f"{section_name}.{variable} length mismatch: expected {len(time_values)}, "
                    f"got {len(values)}."
                ),
            )
        ...
        if variable not in units:
            add_record_issue(
                ...
                section=section_name,
                field=variable,
                message=f"{units_key}.{variable} is missing.",
            )
...
def validate_records(
    *,
    ...
    daily_expected_count = day_count if daily_vars else None
    ...
    for index, record in enumerate(records):
        ...
        record_summary = {
            ...
            "daily": validate_section(
                ...
                start_date_value=start_date_value,
                end_date_value=end_date_value,
                exact_expected_count=daily_expected_count,
                collector=collector,
            ),
        }
...
def add_runtime_overrides(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Safety cap for repeated --daily-var "
            f"(default env {ENV_MAX_DAILY_VARIABLES_PER_RUN})."
        ),
    )
    parser.add_argument(
        "--max-retry-after-seconds",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Open-Meteo historical weather fetch helper.")
    ...
    fetch = sub.add_parser("fetch", help="Fetch historical weather and soil data.")
    ...
    fetch.add_argument(
        ...
        action="append",
        default=[],
        help="Optional model name. Repeat for multiple models.",
    )
    fetch.add_argument(
        "--timezone",
        default="",
    ...
    fetch.add_argument("--temperature-unit", default="", help="Optional temperature unit.")
    fetch.add_argument("--wind-speed-unit", default="", help="Optional wind speed unit.")
    fetch.add_argument("--precipitation-unit", default="", help="Optional precipitation unit.")
    fetch.add_argument(
...
def command_fetch(args: argparse.Namespace) -> int:
    ...
    if day_count > config.max_days_per_run:
        raise ValueError(
            ...
            "Reduce the window or raise OPEN_METEO_MAX_DAYS_PER_RUN explicitly."
        )
    ...
    if not hourly_vars and not daily_vars:
        raise ValueError("Specify at least one --hourly-var or --daily-var.")
    ...
    models = unique_preserve_order(args.model)
    ...
    query = build_request_query(
        ...
        models=models,
        temperature_unit=args.temperature_unit.strip(),
        wind_speed_unit=args.wind_speed_unit.strip(),
        precipitation_unit=args.precipitation_unit.strip(),
    )
    ...
    logger.info(
        "fetch-request locations=%d days=%d hourly_vars=%d daily_vars=%d dry_run=%s url=%s",
        len(locations),
        day_count,
        len(hourly_vars),
        len(daily_vars),
