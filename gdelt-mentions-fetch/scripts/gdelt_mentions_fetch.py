#!/usr/bin/env python3
"""Fetch GDELT 2.0 mentions export files with retries, throttling, and rich logs."""
...
ENV_BASE_URL = "GDELT_MENTIONS_BASE_URL"
...
def build_logger(level: str, log_file: str) -> logging.Logger:
    logger = logging.getLogger("gdelt-mentions-fetch")
...
def as_mentions_entry(size: int | None, md5: str, url: str) -> ExportFileEntry | None:
    match = MENTIONS_URL_PATTERN.search(url)
    if not match:
        return None
...
def parse_lastupdate_mentions(text: str, logger: logging.Logger) -> list[ExportFileEntry]:
    ...
    for line in text.splitlines():
        ...
        if not parsed:
            continue
        size, md5, url = parsed
        entry = as_mentions_entry(size=size, md5=md5, url=url)
        if entry is not None:
            results.append(entry)
    return sorted(results, key=lambda x: x.timestamp)


def iter_masterfile_mentions(
...
def resolve_latest_entries(
    *,
    ...
    entries = parse_lastupdate_mentions(text=text, logger=logger)
    if not entries:
        raise RuntimeError("No mentions export entry found in lastupdate.txt.")
...
def command_resolve_latest(args: argparse.Namespace) -> int:
    ...
    payload = {
        "ok": True,
        "source": "lastupdate",
        "latest_mentions_export": serialize_entry(latest),
...
def select_entries(args: argparse.Namespace, config: RuntimeConfig, logger: logging.Logger) -> list[ExportFileEntry]:
    ...
    candidates = sorted(
        iter_masterfile_mentions(text=text, start=start, end=end, logger=logger),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch GDELT 2.0 mentions export files via lastupdate/masterfilelist."
    )
    ...
    latest = sub.add_parser("resolve-latest", help="Resolve the latest mentions export file.")
    ...
    fetch = sub.add_parser("fetch", help="Download mentions export file(s).")
    ...
    fetch.add_argument(
        "--output-dir",
        default="./data/gdelt-mentions",
        help="Directory for downloaded .mentions.CSV.zip files.",
    )
    fetch.add_argument(
        "--overwrite",
        action="store_true",
    ...
    fetch.add_argument(
        "--expected-columns",
        type=int,
        default=DEFAULT_MENTIONS_EXPECTED_COLUMNS,
        help=f"Expected tab-separated column count for mentions rows. Default: {DEFAULT_MENTIONS_EXPECTED_COLUMNS}.",
    )
    fetch.add_argument(
